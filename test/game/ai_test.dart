...
GameEngine _makeAiEngine() {
  return GameEngine.withPlayers([
    ...
    Player(index: 1, name: 'लाटो बोट', type: PlayerType.ai, difficulty: AiDifficulty.normal),
  ]);
}
...
void main() {
  group('AiEngine', () {
    test('easy: returns a token or null when no moves available', () {
      ...
      final result = AiEngine.chooseMove(engine, 1, 3);
    ...
    test('easy: returns token on a 6 with tokens in yard', () {
      ...
      final result = AiEngine.chooseMove(engine, 1, 6);
    ...
    test('normal: prefers finishing move', () {
      ...
      final result = AiEngine.chooseMove(engine, 1, 4);
    ...
    test('returns null when all tokens are finished', () {
      ...
      final result = AiEngine.chooseMove(engine, 1, 5);
      expect(result, isNull);
    });
  });
}
